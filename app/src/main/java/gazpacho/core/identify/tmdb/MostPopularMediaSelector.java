...
public class MostPopularMediaSelector implements TmdbMediaSelector {
    ...
    private static Optional<Movie> getMostPopularMovie(List<Movie> movies) {
        return CollectionUtils.getFirstInSortedList(new ArrayList<>(movies), getPopularityComparator(movie -> movie.getPopularity()));
    }

    private static Optional<TvSeries> getMostPopularShow(List<TvSeries> tvSeries) {
        return CollectionUtils.getFirstInSortedList(new ArrayList<>(tvSeries), getPopularityComparator(show -> show.getPopularity()));
    }
